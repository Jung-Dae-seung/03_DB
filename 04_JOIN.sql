...
-- 조인되는 테이블의 각 행들이 모두 매피된 데이터가 검색되는 방법 (곱집합)
...
FROM EMPLOYEE
JOIN SAL_GRADE ON(SALARY BETWEEN MIN_SAL AN MAX_SLAL);
...
SUBSTR(EMP_NO, 8, INSTR(EMP_NO, '-') -6)
FROM EMPLOYEE E
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
WHERE EMP_NO LIKE '7%'
AND SUBSTR(EMP_NO, 8, INSTR(EMP_NO, '-') -6) = '2'
AND EMP_NAME LIKE '전%';
