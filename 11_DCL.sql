...
CREATE USER jds_sample IDENTIFIED BY 1234;


-- 2. 새 연결(접속) 추가
-- ORA-01045: 사용자 JDS_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다.
--> 접속 권한
...
-- [권한 부여 작성법]
-- GRANT 권한, 권한,... TO 사용자명;
GRANT CREATE SESSION TO jds_sample;
...
-- CRAETE TABLE : 테이블 생성 권한
-- + 데이터를 저장할 수 있는 공간(TABLESPACE) 할당


-- 6. (sys) 테이블 생성 권한 부여 + TABLESPACE 할당
GRANT CREATE TABLE TO jds_sample;

ALTER USER jds_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
-- (sys) sample 계정에CONNECT, RESOURCE ROLE 부여
GRANT CONNECT, RESOURCE TO jds_sample;
...
SELECT * FROM kh_jds.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
--> 접근 권한이 없어서 조회 불가

--> 2. (kh) sample 계정에 EMPLOYEE 테이블 조회 권한 부여
-- [객체 권한 부여 방법]
-- GRANT SELECT ON EMPLOEE NOT jds_smapl
GRANT SELECT ON EMPLOYEE TO jds_sample;
...
-- 4. (kh) sample 계정에 부여했던 EMPLOYEE 테이블 조회 권한 회수
-- [권한 회수 작성법]
-- REVOKE 객체권한 ON 객체명 FROM 사용자명;
REVOKE SELECT ON EMPLOYEE FROM jds_sample;
